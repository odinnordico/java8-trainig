    ...
    private List<Person> createPersons(final int count) {
        ...
        iterationStream.forEach(iteration -> {
            final Person person = Person.builder()
                    .ocupation(new StringBuilder().append("Ocup").append(" ").append(iteration).toString())
                    .name(new StringBuilder().append("Person").append(" ").append(iteration).toString())
                    .age(new Random().nextInt(70)).build();
            persons.add(person);
        });
        return persons;
    }
